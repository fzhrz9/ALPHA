"""
ALPHA — Gate.io Dual Engine Sniper
Engine 1: Pullback SMC (H1) — 7 Setup Price Action
Engine 2: Early Momentum (M15) — Volume Anomaly + Accumulation
Mode Switcher: /mode pullback | /mode momentum | /mode both
"""
...
# SCAN MODE: pullback | momentum | both
...
STABLECOINS = {
    ...
    "XAUT", "PAXG", "USD", "USDT.BSC", "USDC.BSC", "USDC.BASE",
}
...
def get_gateio_price(sym):
    """Ambil harga terkini untuk satu pair (real-time validation)."""
    try:
        r = requests.get(
...
# =================================================================
# 4. ENGINE 1: SMC + PRICE ACTION (H1) — 7 SETUP (TIDAK DISENTUH)
# =================================================================
def analyze_smc_pa(candles, sym="?", verbose=True):
    ...
    if curr['l'] < swing_low and curr['c'] > swing_low:
        ...
        score += 3
        log(f"✅ SETUP 7 DETECTED: Harga cucuk Swing Low ${fmt(swing_low)}, close semula atas")
    ...
    if is_pinbar:
        ...
        score += 2
        log("✅ SETUP 5 DETECTED: Pinbar dengan ekor panjang")
    elif is_engulfing:
        if not setup_name: setup_name = "🐂 BULLISH ENGULFING"
        score += 2
        log("✅ SETUP 5 DETECTED: Bullish Engulfing")

    # ─── SETUP 4: VPA CONFIRMATION ──────────────────────────
    if vpa_dry:
        score += 1
        log(f"✅ VPA PASS: Volume {curr_vol:,.0f} < 80% avg")
    else:
        log(f"⚠️  VPA WEAK: Volume tinggi ({curr_vol:,.0f})")

    # ─── SETUP 2: TREND PULLBACK (Dekat EMA) ───────────────
    if abs(price - ema20) / ema20 < 0.015:
        if not setup_name: setup_name = "📈 TREND PULLBACK (HH/HL)"
        score += 1
        log("✅ SETUP 2 DETECTED: Pullback ke EMA20")
        ...
        try:
            ...
            if c['c'] < c['o'] and c_next['c'] > c_next['o']:
                ...
                if bos_size > (rng * 0.08) and c['l'] <= price <= c['h']:
                    ...
                    score += 1
                    log(f"✅ SETUP 3 DETECTED: Price dalam Order Block")
                    break
        except:
            pass
    ...
    log(f"🎯 SETUP COMPLETE: {setup_name} | Score: {score} | RR: 1:{rr2:.1f}")
...
# =================================================================
# 5. ENGINE 2: EARLY MOMENTUM (M15) — VOLUME ANOMALY + ACCUMULATION
# =================================================================
def analyze_early_momentum(sym, verbose=True):
    """
    Early Momentum Detection (M15 Timeframe)
    - Volume Anomaly (3x average)
    - Accumulation Pattern (range < 5% selama 50 candle)
    - Bullish Engulfing/Pinbar confirmation
    Returns: dict dengan setup, entry, sl, tp1, tp2, tp3, score ATAU None
    """
    log = lambda msg: print(f"[{sym}-M15] {msg}") if verbose else None

    # Ambil M15 candles (100 candle = 25 jam)
    candles = get_gateio_klines(sym, "15m", 100)
    if len(candles) < 50:
        log("❌ REJECT: Data M15 < 50 candle")
        return None
    ...
    if curr_vol < (avg_vol * 3):
        log(f"❌ REJECT: Volume {curr_vol:,.0f} < 3x avg ({avg_vol:,.0f})")
        return None
    
    vol_spike = curr_vol / avg_vol
    log(f"✅ VOLUME ANOMALY: {vol_spike:.1f}x avg ({curr_vol:,.0f} vs {avg_vol:,.0f})")

    # 2. Accumulation Pattern (range < 5% selama 50 candle)
    ...
    if range_pct > 5:
        log(f"❌ REJECT: Range {range_pct:.1f}% > 5% (bukan accumulation)")
        return None
    
    log(f"✅ ACCUMULATION: Range {range_pct:.1f}% (50 candle)")

    # 3. Price Action Confirmation (Engulfing/Pinbar)
    curr = candles[-1]
    prev = candles[-2]
    price = curr['c']
    ...
    if not (is_pinbar or is_engulfing):
        log("❌ REJECT: Tiada price action confirmation")
        return None
    
    setup_name = "️ PINBAR MOMENTUM" if is_pinbar else "🐂 ENGULFING MOMENTUM"
    log(f"✅ PRICE ACTION: {setup_name}")

    # 4. Kira Entry/SL/TP
    entry = price
    sl = min(lows[-20:]) * 0.99  # SL bawah recent low
    tp1 = max(highs[-50:]) * 1.02  # TP1 at recent high + 2%
    tp2 = entry + (entry - sl) * 3  # TP2 = 3x risk
    tp3 = entry + (entry - sl) * 5  # TP3 = 5x risk
    ...
    log(f" MOMENTUM SETUP: {setup_name} | Vol: {vol_spike:.1f}x | RR: 1:{rr2:.1f}")
    ...
    return {
        ...
        "tp1": tp1, "tp2": tp2, "tp3": tp3,
        "rr1": rr1, "rr2": rr2,
        "score": 3,  # Default score untuk momentum
        "fib_zone": "N/A",
        "swing_high": max(highs),
        "swing_low": min(lows),
        "timeframe": "M15",
...
# =================================================================
# 6. SIGNAL GENERATOR (MINIMALIST + TIMEFRAME DISPLAY)
# =================================================================
def send_signal(sym, smc_data, vol_24h, btc_chg=0.0):
    ...
    if current_price > 0:
        ...
        if price_gap > 2.0:
            print(f"[SKIP] {sym}: Harga dah bergerak {price_gap:.1f}% — signal stale")
            return False

    # Kira % kerugian/keuntungan
    ...
    # Pair name untuk copy
    pair_name = f"{sym}USDT"

    # Icon berdasarkan engine
    if timeframe == "M15":
        engine_icon = "⚡"
    ...
    # Keyboard: Direct Gate.io & TradingView
    ...
    # SIGNAL FORMAT — Timeframe display instead of RR
    msg = (
    ...
    # Tambah info khas untuk momentum
    ...
    try:
        ...
        save_signal(record)
        add_cooldown(sym)
        print(f"[SIGNAL SENT ✅] {sym} | {smc_data['setup']} | {timeframe} | Score: {smc_data['score']}")
        return True
    except Exception as e:
...
# =================================================================
# 7. SCANNER & TRADE MONITOR (DUAL ENGINE)
# =================================================================
def scan_once():
    """Satu kitaran scan menggunakan Gate.io API (Dual Engine)."""
    if not IS_SCANNING:
        return
    ...
    print(f"\n{'='*60}")
    print(f"🔍 [{datetime.now().strftime('%H:%M:%S')}] SCAN DIMULAKAN | Mode: {SCAN_MODE.upper()} | Preset: {preset_lbl}")
    print(f"{'='*60}")
    ...
    print(f"[SAFETY NET] {len(candidates)} pairs lulus Min Vol ${cfg['min_vol_24h']/1e6:.1f}M | {rejected_vol} ditolak")
    ...
    for t in candidates:
        ...
        # ── SKIP STABLECOIN/WRAPPED/LEVERAGED TOKEN ─────────────
        ...
        # Semak cooldown dengan OVERRIDE
        if is_in_cooldown(sym):
            if check_cooldown_override(sym, current_price):
                print(f"[{sym}] 🔄 OVERRIDE: Cooldown di-reset")
            else:
        ...
        if SCAN_MODE in ["momentum", "both"]:
            # Skip jika dah hantar signal pullback untuk coin yang sama
    ...
    print(f"⏭️  Skip: Blacklisted={skipped_reasons['blacklisted']}, Cooldown={skipped_reasons['cooldown']}, Active={skipped_reasons['active']}, NoSetup={skipped_reasons['no_setup']}, ScoreLow={skipped_reasons['score_low']}")
    print(f"{'='*60}\n")
...
def cmd_start(msg):
    ...
    text = (
        ...
        f"└ Timeframe   : <code>H1 (Pullback) + M15 (Momentum)</code>\n\n"
        f"<b>🧠 Engine 1 — Pullback (H1):</b>\n"
        ...
        f"1. Volume Anomaly (3x avg)\n"
        f"2. Accumulation Pattern\n"
        f"3. Early BOS Trigger"
    )
...
def cb_mode(call):
    ...
    if new_mode in ["pullback", "momentum", "both"]:
        ...
        bot.send_message(call.message.chat.id, "❌ Mode tidak sah")
...
def cmd_pair(msg):
    ...
    def _do():
        # Test both engines
        candles_h1 = get_gateio_klines(sym, "1h", 50)
        smc_h1 = analyze_smc_pa(candles_h1, sym, verbose=False) if len(candles_h1) >= 30 else None
        ...
        if smc_h1:
            bot.send_message(msg.chat.id,
                ...
                f"Zone: <code>{smc_h1['fib_zone']}</code>",
                parse_mode="HTML")
        elif smc_m15:
            bot.send_message(msg.chat.id,
                ...
                f"Range: <code>{smc_m15['range_pct']:.1f}%</code>",
                parse_mode="HTML")
        else:
            bot.send_message(msg.chat.id, f"❌ {sym}: Tiada setup dalam kedua-dua engine", parse_mode="HTML")
...
def cmd_help(msg):
    ...
    bot.reply_to(msg, (
        ...
        "/pair [SYM]     — Analisis manual (kedua-dua engine)\n"
        ...
        "/tune hard      — Ketat\n\n"
        "⚡ <b>SCAN MODE:</b>\n"
        "/mode pullback  — Engine 1 sahaja (H1 SMC)\n"
        "/mode momentum  — Engine 2 sahaja (M15)\n"
        "/mode both      — Kedua-dua engine\n\n"
