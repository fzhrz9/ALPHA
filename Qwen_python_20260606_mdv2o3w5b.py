...
FIXES APPLIED:
- Scan interval: 15min → 5min (kurang lagging)
- Major swing: 30 candle → 200 candle (big trader standard)
- Real-time price validation sebelum hantar signal
- Copyable pair name (BABYUSDT) + hyperlink
- Cooldown override (second chance entry)
- Stablecoin/wrapped/leveraged blocklist
"""
...
def get_gateio_klines(sym, interval="1h", limit=200):
    """Ambil Candle dari Gate.io (default 200 untuk major swing)"""
...
def analyze_smc_pa(candles, sym="?", verbose=True):
    ...
    Major Swing: 200 candle H1 = 8 hari (big trader standard)
    ...
    # MAJOR SWING: 200 candle H1 = 8 hari (selari dengan big trader)
...
def send_signal(sym, smc_data, vol_24h, btc_chg=0.0):
    ...
    # REAL-TIME PRICE VALIDATION — elak signal lagging
...
def scan_once():
    ...
    passed = 0
    skipped_reasons = {"cooldown": 0, "active": 0, "no_data": 0, "no_setup": 0, "score_low": 0, "blacklisted": 0, "stale": 0}
    ...
    for t in candidates:
        ...
        # Ambil H1 Candles (200 candle untuk major swing)
        ...
        # Hantar signal dengan BTC warning + real-time validation
...
def cmd_start(msg):
    ...
    text = (
        ...
        f"└ Timeframe   : <code>H1 (200 candle major swing)</code>\n\n"
        f"<b>🧠 Enjin Setup (7):</b>\n"
...
# =================================================================
def run_scheduler():
    # FIX: Scan interval 15min → 5min (kurang lagging)
