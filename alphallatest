...
# ================================================================
# CG_CACHE — Penimbal API CoinGecko (jimat 98% kredit)
# CoinGecko hanya dipanggil 1x setiap 4 jam, bukan setiap 10 minit
# ================================================================
CG_CACHE = {
    'categories':  None,  # senarai kategori trending
    'coins':       {},    # {cat_id: [coin_list]}
    'cached_at':   0,     # timestamp cache disimpan
    'ttl_seconds': 14400, # 4 jam = 14,400 saat
}

def cg_cache_valid():
    """Semak sama ada cache masih segar (< 4 jam)."""
    return (time.time() - CG_CACHE['cached_at']) < CG_CACHE['ttl_seconds']

def cg_cache_status():
    """Laporan ringkas cache untuk admin."""
    if CG_CACHE['cached_at'] == 0:
        return "Belum ada cache"
    age_m = int((time.time() - CG_CACHE['cached_at']) / 60)
    exp_m = int((CG_CACHE['ttl_seconds'] - (time.time() - CG_CACHE['cached_at'])) / 60)
    n_cats = len(CG_CACHE.get('coins', {}))
...
CORE_NARRATIVES = [
    ...
    'solana-ecosystem', 'base-ecosystem', 'bitcoin-ecosystem', 'gaming',
]
...
def get_trending_categories():
    """
    Pulangkan 3 kategori trending.
    Guna cache jika segar (< 4 jam) — elak API call sia-sia.
    """
    if CG_CACHE['categories'] and cg_cache_valid():
        return CG_CACHE['categories']
    try:
        ...
        result = [c['id'] for c in sorted(
            cats, key=lambda x: x.get('market_cap_change_24h', 0) or 0, reverse=True
        )[:3]]
        CG_CACHE['categories'] = result
        # Jangan update cached_at di sini — biar refresh_cg_cache() yang urus
        return result
    except:
        return CG_CACHE.get('categories') or []

def get_coins_in_category(cat_id, per_page=50):
    """
    Pulangkan senarai koin dalam kategori.
    Guna cache jika segar — jimat kredit CoinGecko.
    Jika cache lapuk atau tiada, panggil API dan simpan.
    """
    if cat_id in CG_CACHE['coins'] and cg_cache_valid():
        return CG_CACHE['coins'][cat_id]
    try:
        h = {"x-cg-demo-api-key": CG_API_KEY}
        ...
        CG_CACHE['coins'][cat_id] = result
        return result
    except:
        return CG_CACHE['coins'].get(cat_id, [])

def refresh_cg_cache():
    """
    Muat semula semua data CoinGecko sekarang.
    Dipanggil setiap 4 jam oleh scheduler — bukan semasa scan.
    Selepas ini, semua kitaran 10-minit akan guna cache sahaja.
    Anggaran: 10 sektor × 1 call + 1 trending = 11 calls setiap 4 jam
    = 66 calls/hari (berbanding 2,016 tanpa cache = jimat 97%)
    """
    print("[CG_CACHE] Menyegar semula cache CoinGecko...")
    n_calls = 0

    # 1. Refresh trending categories
    try:
        h = {"x-cg-demo-api-key": CG_API_KEY}
        cats = requests.get(
            "https://api.coingecko.com/api/v3/coins/categories",
        ...
        CG_CACHE['categories'] = [c['id'] for c in sorted(
            cats, key=lambda x: x.get('market_cap_change_24h', 0) or 0, reverse=True
        )[:3]]
        n_calls += 1
    except Exception as e:
        print(f"[CG_CACHE] Gagal trending: {e}")

    # 2. Refresh semua core narratives + trending
    all_cats = list(CORE_NARRATIVES) + (CG_CACHE.get('categories') or [])
    for cat in all_cats:
        try:
            h = {"x-cg-demo-api-key": CG_API_KEY}
            url = (f"https://api.coingecko.com/api/v3/coins/markets"
                   f"?vs_currency=usd&category={cat}"
                   f"&order=market_cap_desc&per_page=50&page=1")
            r = requests.get(url, headers=h, timeout=10).json()
            if isinstance(r, list):
                CG_CACHE['coins'][cat] = r
                n_calls += 1
            time.sleep(1.2)   # hormat rate limit CoinGecko (50 req/min demo)
        except Exception as e:
            print(f"[CG_CACHE] Gagal {cat}: {e}")

    CG_CACHE['cached_at'] = time.time()
    print(f"[CG_CACHE] Selesai. {n_calls} API calls. Cache sah 4 jam.")
    alert_admin(
        f"🗄️ CoinGecko cache dikemas kini\n"
        f"{n_calls} calls | Sah 4 jam\n"
        f"{len(CG_CACHE['coins'])} kategori disimpan"
    )
...
def verify_security(network, contract):
    ...
    try:
        ...
        if buy_tax  > 15: return False, f"🚨 Buy Tax {buy_tax:.0f}% (terlalu tinggi)"
        if sell_tax > 15: return False, f"🚨 Sell Tax {sell_tax:.0f}% (terlalu tinggi)"

        # ── Notis sahaja — TIDAK menyekat signal ────────────────
        # is_proxy: corak kontrak upgradeable — BIASA pada token sah
        #           (USDC, USDT dan ratusan projek lain guna proxy)
        # is_mintable: boleh cetak token baru — amaran, bukan larangan
        notices = []
        ...
        if notices:
            return True, f"⚠️ Notis: {', '.join(notices)} (GoPlus)"
...
def cmd_cache(msg):
    if str(msg.chat.id) != str(ADMIN_ID): return
    bot.reply_to(msg, f"🗄️ <b>CG CACHE STATUS</b>\n{cg_cache_status()}", parse_mode="HTML")

@bot.message_handler(commands=['refreshcache'])
def cmd_refresh_cache(msg):
    if str(msg.chat.id) != str(ADMIN_ID): return
    bot.reply_to(msg, "♻️ Menyegar semula cache CoinGecko...")
    threading.Thread(target=refresh_cg_cache).start()
...
# ================================================================
def run_scheduler():
    # Cache refresh setiap 4 jam — SATU-SATUNYA masa CoinGecko API dipanggil
    schedule.every(4).hours.do(
        lambda: threading.Thread(target=refresh_cg_cache).start()
    )
...
if __name__ == "__main__":
    ...
    # Isi CoinGecko cache dahulu sebelum scan pertama
    print('[STARTUP] Mengisi CoinGecko cache...')
    threading.Thread(target=refresh_cg_cache).start()
    time.sleep(15)  # bagi masa 15s untuk cache siap
