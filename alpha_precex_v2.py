...
DEFAULT_CONFIG = {
    "mc_min":            1_000_000,    # $1M
    "mc_max":          100_000_000,    # $100M
    "liq_min":             100_000,    # $100K
    "vol_mc_ratio_min":      0.05,     # 5%
    "change_24h_min":         2.0,     # +2%
    "atr_sl_mult":            1.5,     # SL = ATR × 1.5
    "cooldown_hours":          24,     # 24 jam
    "min_token_age_days":       3,     # token mesti wujud > 3 hari
    "score_pass":               2,     # lulus jika >= 2 dari 4 (longgar dulu)
}
    ...
    for ca, meta in candidates.items():
        try:
            ...
            if not dex or not dex.get("contract"):
                print(f"[NO DATA] {ca[:16]}... ({meta.get('chain','?')}) — tiada data DEX")
                continue

            # Pre-filter awal (cepat, tanpa API tambahan)
            ok, reason, _ = pre_filter(dex)
            if not ok:
                print(f"[TOLAK] {dex.get('symbol','?')} ({dex.get('network','?')}) | {reason}")
                continue

            # Semak cooldown (Supabase)
            if is_in_cooldown(ca):
                print(f"[COOLDOWN] {dex.get('symbol','?')} — dalam 24j cooldown")
                continue

            # Semak trade aktif
            if is_active_trade(ca):
                print(f"[AKTIF] {dex.get('symbol','?')} — trade masih terbuka")
                continue

            # CEX check (serentak, < 5 saat)
            cex = check_all_cex(dex["symbol"])
            if cex["listed_on_any"]:
                print(f"[SKIP] {dex.get('symbol','?')} — sudah di CEX: {cex['label']}")
                continue

            # CoinGecko listing verification (1 API call)
            if not verify_cg_listed(dex["symbol"]):
                print(f"[SKIP] {dex.get('symbol','?')} — tiada rekod CoinGecko")
                continue
            ...
            if "Overbought" in rsi_lbl:
                print(f"[RSI] {dex.get('symbol','?')} — {rsi_lbl}, skip")
                continue
