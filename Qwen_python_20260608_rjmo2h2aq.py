"""
ALPHA — Gate.io Quantitative Master Sniper (Unified Default)
Engine 1: Pullback SMC (H1 + H4) — Fractal Swing + EMA + VPA Impulse
Engine 2: Early Momentum (M15) — Volume Anomaly + Accumulation
Risk Management: Unified ATR + Dynamic R:Multiple (1R, 2R, 3R) Default
"""
...
# Global Watchlists (Dideklarasikan dengan betul untuk elak NameError)
PULLBACK_WATCHLIST = {}
BOS_WATCHLIST = {}
WATCHLIST = {}

# ⏱️ TIMEOUT: 8 JAM (28,800 saat) untuk memberi ruang retest yang sah
WATCHLIST_TIMEOUT = 28800  
PULLBACK_TIMEOUT = 28800   
BOS_TIMEOUT = 28800        

IS_SCANNING = True

# 🛡️ UNIFIED QUANTITATIVE DEFAULT (Gabungan Terbaik Tanpa Perlu Pilih)
# Menggabungkan ATR (elak stop hunt) + Dynamic R:R (expectancy positif)
UNIFIED_CONFIG = {
    "min_score": 2,          # Benarkan setup berkualiti masuk, tapi tapis sampah (score 1)
    "atr_sl_mult": 1.2,      # SL = 1.2x ATR (Cukup luas untuk elak noise, cukup ketat untuk R:R baik)
    "label": "⚖️ UNIFIED QUANT DEFAULT (ATR + Dynamic R:R)"
}

def alert_admin(text):
    try:
        bot.send_message(ADMIN_ID, f"🚨 <b>ALPHA QUANT SYSTEM</b>\n<pre>{str(text)[:800]}</pre>", parse_mode="HTML")
    except Exception:
        pass

# =================================================================
# 2. PRESETS & SUPABASE HELPERS (SYNTAX DIPERBETULKAN: TIADA SPACE)
# =================================================================
PRESETS = {
...
def apply_preset(preset_name):
    ...
    print(f"🔄 [SYSTEM] PRESET DIUBAH ke: {preset_name.upper()} ({p['label']})")
    return True, p["label"]
...
# =================================================================
# 4. ENGINE 1: PULLBACK SMC (H1 + H4) — UNIFIED QUANT SL/TP
# =================================================================
def analyze_smc_pa(sym, verbose=True):
    ...
    is_pinbar = (lower_wick > (body * 2) and upper_wick < (total_range * 0.1) and curr['c'] > curr['o'] and total_range > (atr * 0.5) if atr > 0 else True)
    prev_body = abs(prev['c'] - prev['o'])
    curr_body = abs(curr['c'] - curr['o'])
    is_engulfing = (curr['c'] > curr['o'] and prev['c'] < prev['o'] and curr['c'] > prev['o'] and curr['o'] <= prev['c'] and curr_body > prev_body and curr_vol > prev['v'])
    ...
    # 🚨 UNIFIED QUANTITATIVE SL/TP ENGINE (ATR + R-Multiple)
    if not setup_name or score < 2:
        # Setup berkualiti (score >= 2) tapi gagal. Kembalikan data untuk watchlist jika perlu.
        return {"setup": setup_name, "entry": price, "score": score, "fib_786": fib_786, "swing_high": swing_high, "timeframe": "H1", "structure": structure, "vpa_dry": vpa_dry, "h4_structure": h4_structure if len(candles_h4)>=20 else 'unknown', "is_reject": True}

    # 1. Kira Structural SL
    structural_sl = min(curr['l'], swing_low) * 0.995
    # 2. Kira ATR-based SL (Dynamic Volatility)
    atr_based_sl = price - (atr * UNIFIED_CONFIG["atr_sl_mult"])
    # 3. Ambil SL yang paling selamat (paling rendah untuk long)
    final_sl = min(structural_sl, atr_based_sl)
    
    risk_unit = price - final_sl
    if risk_unit <= 0:
        log("❌ REJECT: Risk invalid")
        return None

    # 4. Dynamic R:Reward TP
    tp1 = price + (1 * risk_unit)
    tp2 = price + (2 * risk_unit)
    tp3 = price + (3 * risk_unit)
    rr1, rr2 = 1.0, 2.0

    log(f"✅ SETUP COMPLETE: {setup_name} | Score: {score} | SL: {fmt(final_sl)} | 1R: {fmt(risk_unit)}")

    global BOS_WATCHLIST
    shs_list = [s for s in swings if s['type'] == 'SH']
    sls_list = [s for s in swings if s['type'] == 'SL'] # SYNTAX DIPERBETULKAN: 's for s'
    ...
    return {
        "setup": setup_name, "entry": price, "sl": final_sl,
        "tp1": tp1, "tp2": tp2, "tp3": tp3,
        "rr1": rr1, "rr2": rr2, "score": score,
        "fib_zone": fib_zone, "timeframe": "H1", "structure": structure,
        "vpa_dry": vpa_dry, "h4_structure": h4_structure if len(candles_h4)>=20 else 'unknown', "is_reject": False
    }

# =================================================================
# 5. ENGINE 2: EARLY MOMENTUM (M15) — UNIFIED QUANT SL/TP
# =================================================================
def analyze_early_momentum(sym, verbose=True):
    ...
    # FIX 2: TREND ALIGNMENT (H1 Filter)
    candles_h1 = get_gateio_klines(sym, "1h", 50) # SYNTAX DIPERBETULKAN: candles_h1
    h4_bias = "neutral"
    if len(candles_h1) >= 20:
        h1_closes = [c['c'] for c in candles_h1[-20:]]
        h1_ema20 = sum(h1_closes) / 20
        if candles_h1[-1]['c'] < h1_ema20:
            return None # Skip, H1 downtrend

    avg_vol = sum(c['v'] for c in candles[-20:-1]) / 19 if len(candles) >= 20 else 1
    curr_vol = candles[-1]['v']

    if curr_vol < (avg_vol * 2.0): # Dikurangkan ke 2.0x untuk lebih sensitif
        return None
    ...
    if range_pct > 8.0: # Dilembutkan ke 8.0%
        return None
    log(f"✅ ACCUMULATION: Range {range_pct:.1f}%")

    curr = candles[-1]
    prev = candles[-2] # SYNTAX DIPERBETULKAN: prev
    
    body = abs(curr['c'] - curr['o'])
    lower_wick = min(curr['o'], curr['c']) - curr['l']
    
    is_pinbar = (lower_wick > (body * 2.0) and curr['c'] > curr['o'] and curr['c'] > (curr['h'] + curr['l']) / 2)
    ...
    setup_name = "⚡ M15 PINBAR SNIPER" if is_pinbar else "⚡ M15 ENGULFING SNIPER"
    entry = curr['c']
    
    # UNIFIED QUANT SL/TP for M15
    atr = calculate_atr(candles, 14)
    structural_sl = min(lows[-20:]) * 0.995
    atr_based_sl = entry - (atr * UNIFIED_CONFIG["atr_sl_mult"])
    final_sl = min(structural_sl, atr_based_sl)
    
    risk_unit = entry - final_sl
    if risk_unit <= 0: return None

    log(f"⚡ M15 SNIPER SETUP: {setup_name} | Vol: {vol_spike:.1f}x")

    return {
        "setup": setup_name, "entry": entry, "sl": final_sl,
        "tp1": entry + (1 * risk_unit), "tp2": entry + (2 * risk_unit), "tp3": entry + (3 * risk_unit),
        "rr1": 1.0, "rr2": 2.0, "score": 3,
        "fib_zone": "N/A", "timeframe": "M15",
        "vol_spike": vol_spike, "range_pct": range_pct,
        "h4_structure": "neutral", "vpa_dry": True, "is_reject": False
    }
...
def add_pullback_watchlist(sym, smc_data):
    global PULLBACK_WATCHLIST
    if smc_data.get("score", 0) >= 2:
        PULLBACK_WATCHLIST[sym] = {
            "entry": smc_data["entry"], "fib_786": smc_data.get("fib_786", smc_data["sl"]),
            "swing_high": smc_data.get("swing_high", smc_data.get("tp1", smc_data["entry"] * 1.05)),
            "added": time.time(), "setup": smc_data["setup"], "timeframe": smc_data.get("timeframe", "H1"),
            "structure": smc_data.get("structure", "unknown"), "vpa_dry": smc_data.get("vpa_dry", False),
            "h4_structure": smc_data.get("h4_structure", "unknown")
        }
        print(f"[{sym}] 📌 MASUK WATCHLIST (8 Jam): Setup berkualiti, tunggu retest di zon {fmt(smc_data.get('fib_786', smc_data['sl']))} - {fmt(smc_data['entry'])}")

def send_signal(sym, smc_data, vol_24h, btc_chg=0.0):
    if smc_data.get("is_reject", False):
        add_pullback_watchlist(sym, smc_data)
        return False

    cfg = get_config()
    if smc_data["score"] < UNIFIED_CONFIG["min_score"]:
        return False
    ...
    if current_price > 0:
        ...
        if len(candles_atr) >= 15:
            ...
            atr_pct = (sum(trs) / len(trs) / entry) * 100
        ...
        if price_gap > threshold:
            ...
            add_pullback_watchlist(sym, smc_data) # 🚨 QUANT FIX: Simpan setup berkualiti yang terlepas bas
            return False 
    ...
    grade_map = {2: "C+ (60%)", 3: "B+ (75%)", 4: "A (90%)", 5: "S (95%+)"}
    grade = grade_map.get(smc_data["score"], "C (50%)")

    h4_emoji = "🟢 Up" if smc_data.get("h4_structure") == "uptrend" else "🔴 Down" if smc_data.get("h4_structure") == "downtrend" else "🟡 Side"
    h1_emoji = "🟢 Up" if smc_data.get("structure") == "uptrend" else "🔴 Down" if smc_data.get("structure") == "downtrend" else "🟡 Side"
    m15_emoji = "🔴 Break" if timeframe == "M15" else "🟡 Range"
    vpa_status = "✅ Dry (Strong)" if smc_data.get("vpa_dry") else "⚠️ Weak"

    msg = (
        f"{engine_icon} <b>{sym}/USDT — {smc_data['setup']}</b>\n"
        f"⏱ {timeframe} | Grade: {grade}\n\n"
        f"📊 HTF: H4 {h4_emoji} │ H1 {h1_emoji} │ M15 {m15_emoji}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"💰 Entry: {fmt(entry)}\n"
        f"🛑 SL: {fmt(sl)} ({risk_pct:.1f}%)\n\n"
        f"🎯 TP1: {fmt(tp1)} (+{tp1_pct:.1f}%) [RR 1:{smc_data.get('rr1', 1)}]\n"
        f"🎯 TP2: {fmt(tp2)} (+{tp2_pct:.1f}%) [RR 1:{smc_data.get('rr2', 2)}]\n"
        f"🎯 TP3: {fmt(tp3)} (+{tp3_pct:.1f}%) [RR 1:3.0]\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"📈 Confluence:\n"
        f"• Structure: {smc_data.get('structure', 'Unknown').capitalize()}\n"
        f"• Fibonacci: {smc_data.get('fib_zone', 'N/A')}\n"
        f"• VPA: {vpa_status}\n"
        f"• Setup: {smc_data['setup']}\n\n"
        f"📊 Vol: ${vol_24h/1e6:.2f}M {'🟢' if vol_24h > 1_000_000 else '🟡'} │ BTC: {btc_chg:+.2f}% {'🟢' if btc_chg > -2 else '🔴'} │ Risk: {risk_pct:.1f}%"
    )
    ...
    try:
        ...
        save_signal({
            "contract": sym, "symbol": sym, "network": "GATEIO", "entry": entry, "sl": sl,
            "tp1": tp1, "tp2": tp2, "tp3": tp3, "rr1": smc_data.get("rr1", 1), "rr2": smc_data.get("rr2", 2),
            "setup": smc_data["setup"], "fibo_zone": smc_data.get("fib_zone", "N/A"), "score": smc_data["score"],
            "volume_24h": vol_24h, "timeframe": timeframe, "msg_id": sent.message_id, "sent_at": int(time.time())
        })
        add_cooldown(sym)
...
def scan_once():
    ...
    print(f"\n{'='*60}")
    print(f"🔍 [{datetime.now().strftime('%H:%M:%S')}] SCAN | Mode: {SCAN_MODE.upper()} | Config: UNIFIED QUANT DEFAULT")
    print(f"{'='*60}")
    ...
    for t in candidates:
        ...
        if SCAN_MODE in ["momentum", "both"]:
            if t in momentum_candidates:
                ...
                curr = candles_m15[-1]
                current_time = time.time() # SYNTAX DIPERBETULKAN: current_time
                ...
                if curr_vol >= (avg_vol * 2.0):
                    ...
                    range_low = min(lows) # SYNTAX DIPERBETULKAN: range_low
                    ...
                    local_lh = max(recent_highs) if recent_highs else range_high # SYNTAX DIPERBETULKAN: range_high
                    if curr['c'] <= local_lh:
                        if sym in WATCHLIST: del WATCHLIST[sym]
                        continue

                    prev = candles_m15[-2] # SYNTAX DIPERBETULKAN: prev
                    body = abs(curr['c'] - curr['o'])
                    lower_wick = min(curr['o'], curr['c']) - curr['l']
                    ...
                    if range_pct <= 8.0 and (is_pinbar or is_engulfing):
                        smc_m15 = analyze_early_momentum(sym, verbose=False)
                        if smc_m15 and send_signal(sym, smc_m15, t["volume_24h"], btc_chg=btc_chg):
                            passed += 1
                            if sym in WATCHLIST: del WATCHLIST[sym]
                            time.sleep(2)
                else:
...
def cmd_start(msg):
    ...
    text = (
        f"🏴☠️ <b>ALPHA — Quantitative Master Sniper</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"⏱ Uptime   : <code>{uptime_m}m</code>\n"
        f"📊 Active  : <code>{len(active)} trades</code>\n"
        f"🔧 Status  : <code>{'✅ RUNNING' if IS_SCANNING else '⛔ STOPPED'}</code>\n\n"
        f"⚙️ <b>Engine:</b> <code>UNIFIED QUANT DEFAULT</code>\n"
        f"   <i>(ATR + Dynamic R:R 1/2/3)</i>\n"
        f"⚡ <b>Scan Mode:</b> <code>{SCAN_MODE.upper()}</code>\n"
        f"💰 <b>Min Vol:</b> <code>${cfg['min_vol_24h']/1e6:.1f}M</code>"
    )
    ...
    kb.add(
        InlineKeyboardButton("▶️ Start", callback_data="scan_on"),
        InlineKeyboardButton("⏸ Stop", callback_data="scan_off"),
...
def cb_mode(call):
    ...
    if new_mode in ["pullback", "momentum", "both"]:
        ...
        print(f"🔄 [SYSTEM] MODE DIUBAH ke: {new_mode.upper()} melalui Telegram Button")
...
def cmd_tune(msg):
    ...
    if len(parts) < 2:
        ...
        text = (
            ...
            f"🟢 <code>/tune soft</code> — Vol $500K\n"
            f"🟡 <code>/tune standard</code> — Vol $1M\n"
            f"🔴 <code>/tune hard</code> — Vol $2.5M"
        )
...
def cmd_mode(msg):
    ...
    if new_mode in ["pullback", "momentum", "both"]:
        ...
        print(f"🔄 [SYSTEM] MODE DIUBAH ke: {new_mode.upper()} melalui Telegram Command")
        alert_admin(f"⚡ Mode changed: {new_mode.upper()}")
    else:
...
def cmd_pair(msg):
    ...
    def _do():
        ...
        smc_m15 = analyze_early_momentum(sym, verbose=False)
        if smc_h1 and not smc_h1.get("is_reject"):
...
def cmd_status(msg):
    ...
    bot.reply_to(msg, (
        ...
        f"⚙️ Engine: <code>UNIFIED QUANT DEFAULT</code>\n"
...
def cmd_help(msg):
    ...
    bot.reply_to(msg, (
        ...
        "/tune [PRESET]  — Tukar preset volume"
    ), parse_mode="HTML")
...
# =================================================================
# 10. SCHEDULER & WATCHLIST MONITORS (8 JAM, MULTI-REVERSAL)
# =================================================================
def monitor_pullback_watchlist():
    global PULLBACK_WATCHLIST, IS_SCANNING
    ...
    for sym, data in list(PULLBACK_WATCHLIST.items()):
        try:
            ...
            curr = candles_m5[-1]
            prev = candles_m5[-2]
            prev2 = candles_m5[-3] if len(candles_m5) >= 3 else prev
            current_price = curr['c']
            
            # Aggressive Early Entry: Benarkan harga sentuh Entry (Fib 0.50) dengan toleransi 1%, JANGAN bawah SL (Fib 0.786)
            if current_price > (data["entry"] * 1.01) or current_price < data["fib_786"]:
                continue 
            ...
            # MULTI-REVERSAL PATTERN TRIGGER (SALAH SATU = LULUS)
            body = abs(curr['c'] - curr['o'])
            total_range = curr['h'] - curr['l']
            lower_wick = min(curr['o'], curr['c']) - curr['l']
            upper_wick = curr['h'] - max(curr['o'], curr['c'])
            ...
            is_pinbar = (lower_wick > (body * 2.0) and curr['c'] > curr['o'])
            is_engulfing = (curr['c'] > curr['o'] and prev['c'] < prev['o'] and curr['c'] > prev['o'] and curr['o'] <= prev['c'])
            is_doji = (body < (total_range * 0.15) and total_range > 0 and lower_wick > 0)
            is_tweezer = (abs(curr['l'] - prev['l']) / curr['l'] <= 0.005 and curr['c'] > curr['o']) if curr['l'] > 0 else False
            is_morning_star = (prev2['c'] < prev2['o'] and body < (prev_body * 0.5) and curr['c'] > curr['o'] and curr['c'] > (prev2['c'] + prev2['o']) / 2)
            is_harami = (prev['c'] < prev['o'] and curr['c'] > curr['o'] and curr['o'] > prev['c'] and curr['c'] < prev['o'])

            if is_pinbar or is_engulfing or is_doji or is_tweezer or is_morning_star or is_harami:
                if is_tweezer: setup_name = "🔄 TWEEZER RECOVERY (M5)"
                elif is_morning_star: setup_name = "🔄 MORNING STAR (M5)"
                elif is_harami: setup_name = "🔄 HARAMI (M5)"
                elif is_doji: setup_name = "🔄 DOJI (M5)"
                elif is_engulfing: setup_name = "🔄 ENGULFING (M5)"
                else: setup_name = "🔄 PINBAR (M5)"

                smc_pullback = {
                    "setup": setup_name, "entry": curr['c'], "sl": data["fib_786"] * 0.99,
                    "tp1": data["entry"], "tp2": data["entry"] * 1.05, "tp3": data["entry"] * 1.10,
                    "rr1": 1.0, "rr2": 2.0, "score": 4, "fib_zone": "N/A", "timeframe": "M5",
                    "h4_structure": data.get("h4_structure", "unknown"), "vpa_dry": True, "is_reject": False
                }
                if send_signal(sym, smc_pullback, 0, btc_chg=0.0):
                    symbols_to_remove.append(sym)
                    print(f"[{sym}] 🚀 PULLBACK TRIGGERED! ({setup_name}) Signal dihantar.")
        except Exception as e:
    ...
    for sym, data in list(BOS_WATCHLIST.items()):
        try:
            ...
            if bos_type == "HL" and current_price < level:
                ...
                smc_bos = {
                    "setup": "💥 BOS BREAK (Bearish)", "entry": current_price, "sl": level * 1.02,
                    "tp1": current_price * 0.95, "tp2": current_price * 0.90, "tp3": current_price * 0.85,
                    "rr1": 1.0, "rr2": 2.0, "score": 4, "fib_zone": "N/A", "timeframe": "M5",
                    "h4_structure": "unknown", "vpa_dry": True, "is_reject": False
                }
                ...
                smc_bos = {
                    "setup": "💥 BOS BREAK (Bullish)", "entry": current_price, "sl": level * 0.98,
...
class RenderHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"ALPHA QUANT MASTER ACTIVE")
    def do_HEAD(self):
        self.send_response(200)
...
# =================================================================
# 11. MAIN ENTRY POINT (SYNTAX DIPERBETULKAN: __name__ == "__main__")
# =================================================================
if __name__ == "__main__":
    ...
    print(f"🚀 [SYSTEM] ALPHA QUANTITATIVE MASTER DEPLOYED")
    print(f"🚀 [SYSTEM] Mode Aktif: {SCAN_MODE.upper()}")
    print(f"🚀 [SYSTEM] Config: UNIFIED QUANT DEFAULT (ATR + Dynamic R:R)")
    print(f"🚀 [SYSTEM] Watchlist Timeout: 8 JAM (Aggressive Early Entry Enabled)")
    
    alert_admin(
        f"🏴‍☠️ ALPHA QUANT MASTER ONLINE\n"
        f"Mode: {SCAN_MODE.upper()}\n"
        f"Engine: UNIFIED QUANT DEFAULT\n"
        f"Watchlist: 8 Jam"
    )
    threading.Thread(target=scan_once).start()
