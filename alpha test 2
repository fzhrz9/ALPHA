...
# =================================================================
# 2. PRESETS & SUPABASE HELPERS (TIADA TRAILING SPACES)
# =================================================================
PRESETS = {
...
def apply_preset(preset_name):
    ...
    # LOG RENDER: Pastikan preset change dipaparkan
    print(f"🔄 [SYSTEM] PRESET DIUBAH ke: {preset_name.upper()} ({p['label']}) | Min Vol: ${p['min_vol_24h']:,} | Pass Score: {p['score_pass']}")
...
# =================================================================
# 5. ENGINE 2: M15 FAST-TRACK SNIPER (OPTIMIZED)
# =================================================================
def analyze_early_momentum(sym, verbose=True):
    ...
    if ema20 < ema50 or current_price < ema20:
        return None
    ...
    is_pinbar = (lower_wick > (body * 2.0) and 
                 curr['c'] > curr['o'] and 
                 curr['c'] > (curr['h'] + curr['l']) / 2)
...
def add_pullback_watchlist(sym, smc_data):
    ...
    PULLBACK_WATCHLIST[sym] = {
        "entry": smc_data["entry"],
        "fib_500": smc_data.get("tp1", smc_data["entry"] * 1.05),
        "fib_786": smc_data["sl"],
        "added": time.time(),
...
def scan_once():
    ...
    print(f"\n{'='*60}")
    print(f"🔍 [{datetime.now().strftime('%H:%M:%S')}] SCAN | Mode: {SCAN_MODE.upper()} | Preset: {preset_lbl}")
    print(f"{'='*60}")
            ...
            if t in momentum_candidates:
                ...
                if curr_vol >= (avg_vol * 2.0):
                    ...
                    if range_pct <= 8.0 and (is_pinbar or is_engulfing):
                        smc_m15 = {
                            "setup": "⚡ M15 PINBAR SNIPER" if is_pinbar else "⚡ M15 ENGULFING SNIPER",
                            "entry": curr['c'], "sl": min(lows[-20:]) * 0.995,
                            "tp1": max(highs[-50:]) * 1.015,
                            "tp2": curr['c'] + (curr['c'] - min(lows[-20:]) * 0.995) * 2.5,
                            "tp3": curr['c'] + (curr['c'] - min(lows[-20:]) * 0.995) * 4.0,
                            "rr1": 2.5, "rr2": 4.0, "score": 3,
                            "fib_zone": "N/A", "timeframe": "M15",
...
# =================================================================
# 8. TELEGRAM COMMANDS (DENGAN LOG RENDER EKSPRESIF)
# =================================================================
...
def cmd_start(msg):
    ...
    text = (
        ...
        f"Volume Anomaly (2.0x) | Accumulation Pattern (<8%)"
    )
...
def cb_mode(call):
    ...
    if new_mode in ["pullback", "momentum", "both"]:
        ...
        # LOG RENDER: Papar perubahan mode
    ...
    for sym, data in list(BOS_WATCHLIST.items()):
        try:
            ...
            if bos_type == "HL" and current_price < level:
                ...
                smc_bos = {
                    "setup": "💥 BOS BREAK (HL Tembus - Bearish)", "entry": current_price, "sl": level * 1.02,
                ...
                smc_bos = {
                    "setup": "💥 BOS BREAK (LH Tembus - Bullish)", "entry": current_price, "sl": level * 0.98,
...
if __name__ == "__main__":
    ...
    # LOG RENDER: Pengesahan startup
    print(f"🚀 [SYSTEM] ALPHA Dual Engine v2 DEPLOYED (Institutional Grade)")
    print(f"🚀 [SYSTEM] Mode Aktif: {SCAN_MODE.upper()}")
    print(f"🚀 [SYSTEM] Preset Aktif: {preset_lbl} (Min Vol: ${cfg['min_vol_24h']:,} | Pass Score: {cfg['score_pass']})")
