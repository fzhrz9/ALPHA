...
# =====================================================================
# 2. PARAMETER GLOBAL & MEMORI QUANT (WATCHLIST & ANTI-SPAM)
# =====================================================================
IS_SCANNING = True
...
# =====================================================================
# 3. HELPER FUNCTIONS (PREMIUM DECIMAL FORMATTER)
# =====================================================================
def format_crypto_val(val):
    """Format nilai perpuluhan secara dinamik berasaskan saiz nilai koin"""
...
def check_binance_listing(symbol):
    """Semak secara langsung jika koin sudah berdagang di Binance Spot"""
    try:
...
# =====================================================================
# 4. LIVE API FETCHERS (OPTIMIZED FOR DUAL-ENGINE)
# =====================================================================
...
# =====================================================================
# 5. PENAPISAN, VETO & LIVE SECURITY API 
# =====================================================================
def verify_security_live(network, contract_address):
    try:
        if network.lower() in ['solana', 'sol']:
            ...
            return "SECURE (RugCheck)" if score < 500 else "HIGH RISK"
...
# =====================================================================
# 6. ENJIN PENGIRAAN: ATR, RSI & FIBO (WALL STREET FORMULA)
# =====================================================================
def calculate_rsi_fibo_live(network, pair_address, current_live_price):
    try:
        ...
        if len(ohlcv_list) < 14:
            return "Koin Baru", "Data Tidak Mencukupi", 0
...
# =====================================================================
# 7. ALGO TRADE SETUP & PREMIUM EXCLUSIVE UI
# =====================================================================
def send_signal(coin_info, dex_data, verdict="ALPHA ACTIVE", target_chat_id=VIP_CHANNEL_ID):
    ...
    # --- INTELLIGENT ROUTING & SECURITY HIDDEN SYSTEM ---
    is_binance = check_binance_listing(sym)
    is_solana = dex_data['network'].lower() in ['solana', 'sol']
    
    markup = InlineKeyboardMarkup()
    
    if is_binance:
        # Jika ada di Binance: UTAMAKAN Binance, SOROK Maestro & BonkBot
        markup.row(InlineKeyboardButton("🟧 BUY ON BINANCE", url=f"https://www.binance.com/en/trade/{sym}_USDT"))
    else:
        # Jika TIADA di Binance: Ikut Peraturan Sektor Eco
        if is_solana:
            markup.row(InlineKeyboardButton("🔫 BUY VIA BONKBOT", url=f"https://t.me/bonkbot_bot?start={coin_info['contract_address']}"))
        else:
            markup.row(InlineKeyboardButton("🦄 BUY VIA MAESTRO", url=f"https://t.me/maestro?start={coin_info['contract_address']}"))

    # Baris Butang Maklumat Tambahan (Sleek Grid)
    ...
    markup.row(InlineKeyboardButton("X Real-Time Search", url=f"https://twitter.com/search?q=%24{sym}"))

    # --- DESIGN PREMIUM MINIMALIST TEXT ---
...
├ <b>Turnover Ratio :</b> <code>{turnover_ratio:.1f}x Vol/Liq</code>
└ <b>Token Age :</b> <code>{dex_data['age_display']}</code>

┌ <b>QUANT MOMENTUM STRUCTURE</b>
├ <b>Trend (24H) :</b> <code>+{dex_data['price_change_24h']:.2f}%</code>
├ <b>Sniper (5M) :</b> <code>+{dex_data['price_change_5m']:.2f}%</code>
...
• <b>STOP LOSS :</b> <code>${format_crypto_val(sl)}</code> [<code>-{sl_pct:.1f}%</code>] 🚨
• <b>TARGET TP1 :</b> <code>${format_crypto_val(tp1)}</code> [<code>+{tp1_pct:.1f}%</code>] (RR 1:1.5)
• <b>TARGET TP2 :</b> <code>${format_crypto_val(tp2)}</code> [<code>+{tp2_pct:.1f}%</code>] (RR 1:3.0)
• <b>TARGET TP3 :</b> <code>${format_crypto_val(tp3)}</code> [<code>+{tp3_pct:.1f}%</code>] (RR 1:5.0)
...
def process_warm_pool():
    ...
    for sym, timestamp in list(WARM_POOL.items()):
        ...
        if is_passed:
            # Jika sudah bertukar positif, pastikan belum di-spam dalam masa terdekat
            ...
            SENT_POOL[sym] = time.time() # Kunci Cooldown
            to_remove.append(sym)
        elif not is_warm:
            to_remove.append(sym)
...
def clean_cooldown_pool():
    """Membersihkan memori anti-spam yang melebihi 1 jam"""
    global SENT_POOL
    now = time.time()
    ...
    try:
        ...
        for cat in categories:
            ...
            for coin in coins[:max_coins]: 
                ...
                # JIKA KOIN SEDANG DALAM COOLDOWN (ANTI-SPAM), JANGAN IMBASED
                if sym in SENT_POOL and (time.time() - SENT_POOL[sym] < 3600): continue
                ...
                if is_passed:
                    ...
                    SENT_POOL[sym] = time.time() # Tandakan sudah dihantar
                elif is_warm:
...
# =====================================================================
# 9. TELEGRAM COMMANDS & SERVER ENVIRONMENT
# =====================================================================
...
def cmd_ca(message):
    try:
        ...
        if dex_data:
            ...
            send_signal(c_info, dex_data, verdict="MANUAL DD PRO 🔍", target_chat_id=VIP_CHANNEL_ID)
            bot.reply_to(message, "✅ Signal blast manual berjaya!")
        else: 
...
class RenderHandler(BaseHTTPRequestHandler):
    def do_GET(self): self.send_response(200); self.end_headers(); self.wfile.write(b"AlphaV4 PRO PREMIUM EDITION ACTIVE")
...
if __name__ == "__main__":
    ...
    try: bot.send_message(ADMIN_ID, "🚨 ALPHA V4 PRO (PREMIUM EDITION V8.0) DEPLOYED SUCCESSFULLY")
    except: pass
