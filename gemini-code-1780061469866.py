...
# =====================================================================
# 2. PARAMETER GLOBAL & PERSISTENT MEMORY
# =====================================================================
IS_SCANNING = True
WARM_POOL = {}  
ACTIVE_TRADES = {} 
...
def verify_security_live(network, contract_address):
    try:
        ...
        if network in ['solana', 'sol']:
            ...
            # MAP NETWORK UNTUK GOPLUS API SEBENAR
...
def execute_sniper_protocol(dex_data):
    ...
    # SISTEM SKOR SEBENAR (Bermula 0, Wajib Capai 4/5)
    score = 0
    failed_reasons = []
...
def calculate_quant_intraday(network, pair_address, current_live_price):
    try:
        ...
        if len(ohlcv_list) < 14: return "Data <14", "N/A", 0, 0, 0, 0, 0
        ...
        # FIBONACCI 15-MINIT (INTEGRASI PENUH KE TP)
        ...
        fibo_ext = max_high + (0.618 * total_range) # Fibo Extension 1.618
...
def send_signal(coin_info, dex_data, verdict="ALPHA ACTIVE", target_chat_id=VIP_CHANNEL_ID):
    ...
    # Pastikan TP sentiasa logik ke atas (Fail-safe Kuantitatif)
...
class RenderHandler(BaseHTTPRequestHandler):
    def do_GET(self): self.send_response(200); self.end_headers(); self.wfile.write(b"TrueQuant Protocol V10.0 ACTIVE")
...
if __name__ == "__main__":
    ...
    bot.send_message(ADMIN_ID, "🚨 ALPHA V4 PRO (V10.0 TRUE QUANT PROTOCOL) DEPLOYED. MAAF ATAS KESILAPAN LALU.")
