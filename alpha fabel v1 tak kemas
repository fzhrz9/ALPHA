"""
ALPHA — Gate.io Triple Engine Sniper v16.2 SYARAT-5
...
def send_signal(sym, smc_data, vol_24h, btc_chg=0.0):
    ...
    # [v16.2-UI] Checklist 5 syarat dalam mesej signal
    sy_map = smc_data.get("syarat", {})
    if sy_map:
        items = list(sy_map.items())
        sy_lines = "\n".join(
            f"{'└' if i == len(items)-1 else '├'} {'✅' if v else '⬜'} {k}"
            for i, (k, v) in enumerate(items))
        sy_pass = smc_data.get("syarat_pass", 0)
        sy_emoji = "🟢" if sy_pass >= 4 else "🟡"
        sy_block = f"📋 <b>SYARAT {sy_pass}/5</b> {sy_emoji}\n<code>{sy_lines}</code>\n"
    else:
        sy_block = ""
...
def cmd_pair(msg):
    ...
    def _do():
        ...
        if r:
            ...
            sy = r.get('syarat', {})
            sy_txt = "\n".join(f"{'✅' if v else '⬜'} {k}" for k, v in sy.items()) if sy else "—"
            bot.send_message(msg.chat.id, (
                f"<b>{sym} — {eng}</b>\n"
                f"━━━━━━━━━━━━━━━━━\n"
                f"🧠 <code>{safe_html(r['setup'], 50)}</code>\n"
                f"📋 <b>SYARAT {r.get('syarat_pass', 0)}/5</b>\n<code>{safe_html(sy_txt, 200)}</code>\n"
...
class RenderHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"ALPHA v16.2 SYARAT-5 ACTIVE")
    def do_HEAD(self):
        self.send_response(200)
...
if __name__ == "__main__":
    ...
    sess_n, _, sess_e = get_trading_session()
    alert_admin(
        f"🏴‍☠️ ALPHA v16.2 SYARAT-5 DEPLOYED\n"
