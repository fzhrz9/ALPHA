...
def analyze_smc_pa(sym, verbose=True):
    ...
    if not is_uptrend:
        ...
        # [FIX-19] Relax EMA reject bila: sideway + discount zone + EMA converging (<3%)
        # EMA adalah lagging indicator — dalam sideway accumulation, crossover belum berlaku
        # tapi momentum sudah beralih. Gap <3% = EMA dalam proses crossing.
        if structure == 'sideway' and in_discount and ema_gap_pct < 3.0:
            log(f"⚠️ EMA CONVERGING: gap {ema_gap_pct:.2f}% < 3%, sideway+discount → Allow (lagging indicator)")
