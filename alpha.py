...
def check_cooldown_override(contract, current_price):
    try:
        # SYNTAX DIPERBETULKAN: Tiada space dalam string
...
# =================================================================
# SYNTAX DIPERBETULKAN: Tiada space dalam set
...
def analyze_smc_pa(sym, verbose=True):
    ...
    # 1. H4 TREND CONFIRMATION (UPGRADE: WARNING, BUKAN SKIP)
    ...
    # UPGRADE: COUNTER-TREND TP CAPPING
    ...
    sls_list = [s for s in swings if s['type'] == 'SL'] # SYNTAX DIPERBETULKAN
...
def analyze_early_momentum(sym, verbose=True):
    ...
    # UPGRADE: EMA FILTER DIBUANG SEPENUHNYA (M5/M15 noisy)

    avg_vol = sum(c['v'] for c in candles[-20:-1]) / 19 if len(candles) >= 20 else 1
    curr_vol = candles[-1]['v']

    # DIKEKALKAN MACAM ASAL (Minimum Setting)
    ...
    if h4_bias == "downtrend":
        ...
        log("⚠️ HTF BIAS: H4 Downtrend. SL tightened, TP3 capped.")
    else:
...
def send_signal(sym, smc_data, vol_24h, btc_chg=0.0):
    ...
    # UPGRADE: COUNTER-TREND BADGE
    counter_trend_badge = ""
    if smc_data.get("is_counter_trend", False):
        counter_trend_badge = "⚠️ <b>COUNTER-TREND MODE:</b> TP capped at H4 Swing High\n"
    ...
    for t in candidates:
        ...
        if SCAN_MODE in ["momentum", "both"]:
            if t in momentum_candidates:
                ...
                # UPGRADE: H1 FILTER DIBUANG SEPENUHNYA (M15 leading indicator)
                
                candles_m15 = get_gateio_klines(sym, "15m", 100)
                if len(candles_m15) < 50: continue
                
                curr = candles_m15[-1]
                current_time = time.time() # SYNTAX DIPERBETULKAN
                ...
                if curr_vol >= (avg_vol * 3): # DIKEKALKAN MACAM ASAL
                    ...
                    range_low = min(lows) # SYNTAX DIPERBETULKAN
                    ...
                    local_lh = max(recent_highs) if recent_highs else range_high # SYNTAX DIPERBETULKAN
                    if curr['c'] <= local_lh:
                        if sym in WATCHLIST: del WATCHLIST[sym]
                        continue

                    prev = candles_m15[-2] # SYNTAX DIPERBETULKAN
                    body = abs(curr['c'] - curr['o'])
                    lower_wick = min(curr['o'], curr['c']) - curr['l']
                    is_pinbar = lower_wick > (body * 2) and curr['c'] > curr['o']
                    is_engulfing = (curr['c'] > curr['o'] and prev['c'] < prev['o'] and curr['c'] > prev['o'] and curr['o'] < prev['c'])
                    
                    if range_pct <= 5 and (is_pinbar or is_engulfing): # DIKEKALKAN MACAM ASAL
                        smc_m15 = {
...
def monitor_pullback_watchlist():
    ...
    for sym, data in list(PULLBACK_WATCHLIST.items()):
        try:
            ...
            candles_m5 = get_gateio_klines(sym, "5m", 50) # SYNTAX DIPERBETULKAN
            ...
            curr = candles_m5[-1]
            prev = candles_m5[-2] # SYNTAX DIPERBETULKAN
            body = abs(curr['c'] - curr['o'])
            lower_wick = min(curr['o'], curr['c']) - curr['l']
            is_pinbar = lower_wick > (body * 2) and curr['c'] > curr['o']
            is_engulfing = (curr['c'] > curr['o'] and prev['c'] < prev['o'] and curr['c'] > prev['o'] and curr['o'] < prev['c'])
...
# SYNTAX DIPERBETULKAN: __name__ == "__main__"
