"""
ALPHA — Gate.io Triple Engine Sniper v16.0 ULTIMATE
...
MIN_EV             = 0.15    # [OPT-9] Expected Value gate (dalam unit R)
...
def check_whale_proxy(sym, candles_h4, candles_h1):
    ...
    if whale_score == -1:  return "LEAN_BEARISH", whale_score, desc
...
def analyze_smc_pa(sym, verbose=True):
    ...
    if not is_uptrend:
        ...
        if structure == 'sideway' and in_discount and ema_gap_pct < 6.0:   # [OPT] 5→6%
            log(f"⚠️ EMA CONVERGING {ema_gap_pct:.2f}% — allow")
            is_uptrend = True
        else:
    ...
    if price < ema20 * 0.90:
        log("❌ REJECT: Price jauh bawah EMA20")
        return None
    ...
    if strong_count < 1 and conf_score < 3:
        log(f"❌ REJECT: Confluence kurang (strong={strong_count}, total={conf_score})")
        return None
    ...
    # [OPT-8/9] P(win) + EV
    p_win   = estimate_win_probability(conf_score, strong_count)
...
def analyze_early_momentum(sym, verbose=True):
    ...
    # [OPT-1] Z-score ≥ 2.0 ATAU ratio ≥ 2.5 (dulu fixed 3.0x)
    vol_ratio = curr_vol / avg_vol if avg_vol > 0 else 0
    if not (v_z >= 2.0 or vol_ratio >= 2.5):
        return None
    ...
    # [OPT-5] CLV confirm — buyer kawal candle
    if clv(curr) < 0.1:
        log("❌ REJECT: CLV < 0.1 — close lemah")
        return None
...
def analyze_breakout_sniper(sym, verbose=True):
    ...
    if ema50 <= ema200:
        log("❌ REJECT: No Golden Cross")
        return None

    # [BUG-8/OPT-3] ADX ≥ 18 + Wilder directional rule (+DI > -DI)
    adx, plus_di, minus_di = calculate_adx(candles, 14)
    if adx < 18:
        log(f"❌ REJECT: ADX {adx:.1f} < 18")
        return None
    if plus_di <= minus_di:
        log(f"❌ REJECT: +DI ({plus_di:.1f}) ≤ -DI ({minus_di:.1f}) — trend kuat tapi BEARISH")
        return None
    ...
    v_z       = volume_zscore(vols, lookback=20)
    if not (vol_ratio >= 2.0 and v_z >= 1.5):
        log(f"❌ REJECT: Vol ratio {vol_ratio:.1f}x / z {v_z:.2f} tak cukup")
        return None

    # [OPT-2] RSI ≥ 50 + slope > 0 (derivative momentum)
    rsi_now  = calculate_rsi(closes, 14)
    rsi_prev = calculate_rsi(closes[:-1], 14)
    rsi_slope = rsi_now - rsi_prev
    if rsi_now < 50 or rsi_slope <= 0:
        log(f"❌ REJECT: RSI {rsi_now:.1f} slope {rsi_slope:+.2f}")
        return None
    ...
    session_name, session_score, session_emoji = get_trading_session()
    if session_score < 0:
        log("❌ REJECT: Dead zone — tiada follow-through")
        return None
    ...
    log(f"✅ BREAKOUT | ADX {adx:.1f} +DI>{minus_di:.0f} | Vol {vol_ratio:.1f}x z{v_z:.1f} | RSI {rsi_now:.1f}↑ | {session_emoji}{session_name} | 🐳{whale_bo}")
...
def send_signal(sym, smc_data, vol_24h, btc_chg=0.0):
    ...
    if rr1_net < 1.3 and rr2_net < 1.3:
        print(f"[SKIP] {sym}: RRnet1={rr1_net:.2f} RRnet2={rr2_net:.2f} < 1.3")
        return False
    ...
    ct_badge = "⚠️ <b>COUNTER-TREND</b> — TP capped\n" if smc_data.get("is_counter_trend") else ""
    ...
    inst = f"{se} · {we}"
    if choch not in ("NONE", None):       inst += f" · 🔄{choch.replace('CHOCH_','')}"
    if sweep_conf not in ("NONE", "", None): inst += f" · 💧{sweep_conf}"

    qbar    = quality_bar(conf_score, 10)
    q_emoji = "🟢" if conf_score >= 7 else ("🟡" if conf_score >= 4 else "🔴")
    ...
    msg = (
        ...
        f"🎯 TP2    <code>${fmt(tp2)}</code>  <i>(+{tp2_pct:.1f}% · RR {rr2:.1f})</i>\n"
        f"🎯 TP3    <code>${fmt(tp3)}</code>  <i>(+{tp3_pct:.1f}%)</i>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"📊 Quality  <code>{qbar}</code> {conf_score}/10 {q_emoji}\n"
        f"🎲 P(win)≈<b>{p_win*100:.0f}%</b> · EV <b>{ev:+.2f}R</b> · RRnet <b>{rr1_net:.2f}</b>\n"
        f"🏛️ {inst}\n"
        f"🔗 <code>{safe_html(conf_labels, 70)}</code>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"💼 <code>${pos_usd:.2f}</code> ({pos_coins:.4f} {sym}) · Risk <code>${risk_usd:.2f}</code> ({user_risk}%)\n"
        f"🧠 <code>{safe_html(smc_data['setup'], 60)}</code> · TF <code>{timeframe}</code> · Score <code>{smc_data['score']}</code>"
    )
    if timeframe == "M15" and "vol_spike" in smc_data:
        msg += f"\n⚡ Vol <code>{smc_data['vol_spike']:.1f}x</code> (z={smc_data.get('vol_z', 0):.1f})"
...
def scan_once():
    ...
    try:
        ...
        print(f"[GATEIO] {len(tickers)} pairs → {len(candidates)} candidates")
        ...
        print(f"\n📊 SCAN SELESAI | {passed} signal | {sess_n}\n{'='*65}\n")
    finally:
        with _scan_lock:
            IS_SCANNING_ACTIVE = False

def monitor_active_trades():
    """[BUG-13] Guna HIGH/LOW candle + SL-first conservative rule."""
    active = get_active_trades()
    ...
    for sym, trade in active.items():
        try:
            ...
            def notify(text, _mid=mid):
                ...
                try: bot.send_message(VIP_CHANNEL_ID, full, **kw)
                except Exception: bot.send_message(VIP_CHANNEL_ID, full, parse_mode="HTML")
            ...
            if sl_touched and not trade.get("sl_hit") and not trade.get("tp1_hit"):
                ...
                if hi >= trade["tp3"] and not trade.get("tp3_hit"):
                    ...
                    updates["closed"]  = True
                    p = (cp - entry_price) / entry_price * 100
                    notify(f"🏆 <b>TP3 MOONSHOT!</b>\n💰 <code>${fmt(cp)}</code> | +{p:.2f}%")
                # SL selepas TP1 = breakeven logic (cp bawah entry selepas tp1_hit)
                if trade.get("tp1_hit") and cp <= entry_price and not trade.get("closed") and not trade.get("be_hit"):
                    updates["be_hit"] = True
                    updates["closed"] = True
                    notify(f"🔒 <b>BREAKEVEN EXIT</b>\n💰 <code>${fmt(cp)}</code> (selepas TP1)")
            if updates:
...
def cmd_tune(msg):
    ...
    if len(parts) < 2:
        ...
        bot.reply_to(msg, (
            ...
            f"🟢 <code>/tune soft</code> — $500K, pass 2\n"
            f"🟡 <code>/tune standard</code> — $1M, pass 2\n"
            f"🔴 <code>/tune hard</code> — $2.5M, pass 3\n\n"
...
def cmd_pair(msg):
    ...
    def _do():
        ...
        if not r:
            ...
            eng = "⚡ MOMENTUM (M15)"
        if r:
            qb = quality_bar(r.get('confluence_score', 0), 10)
            bot.send_message(msg.chat.id, (
                f"<b>{sym} — {eng}</b>\n"
                f"━━━━━━━━━━━━━━━━━\n"
                f"🧠 <code>{safe_html(r['setup'], 50)}</code>\n"
                f"📊 <code>{qb}</code> {r.get('confluence_score', 0)}/10\n"
                f"🎲 P(win)≈<b>{r.get('p_win', 0)*100:.0f}%</b> · EV <b>{r.get('ev', 0):+.2f}R</b>\n"
                f"💰 Entry <code>${fmt(r['entry'])}</code> · SL <code>${fmt(r['sl'])}</code>\n"
                f"🎯 RRnet <code>{r.get('rr1_net', 0):.2f}</code> · Score <code>{r['score']}</code>\n"
                f"🏛️ <code>{r.get('session', '?')}</code> · 🐳 <code>{r.get('whale_signal', '?')}</code> · 🔄 <code>{r.get('choch', '?')}</code>\n"
                f"🔗 <code>{safe_html(r.get('confluence_labels', ''), 60)}</code>"
            ), parse_mode="HTML")
        else:
...
def _send_status(chat_id):
    ...
    bot.send_message(chat_id, (
        ...
        f"Vol ≥ <code>${cfg['min_vol_24h']/1e6:.1f}M</code> · Pass <code>{cfg['score_pass']}</code> · EV ≥ <code>{cfg.get('min_ev', MIN_EV)}R</code>"
    ), parse_mode="HTML")
...
def generate_journal():
    ...
    closed = tp1_n + sl_n
    wr = tp1_n / closed * 100 if closed else 0

    def breakdown(key):
        d = {}
        for t in trades:
            k = str(t.get(key, "?"))[:14]
            hit = 1 if t.get("tp1_hit") else 0
            cls = 1 if (t.get("tp1_hit") or t.get("sl_hit")) else 0
            if k not in d: d[k] = [0, 0, 0]
            d[k][0] += 1; d[k][1] += hit; d[k][2] += cls
        out = []
        for k, (n, w, c) in sorted(d.items(), key=lambda x: -x[1][0])[:5]:
            wrk = f"{w/c*100:.0f}%" if c else "—"
            out.append(f"{k}: {n} (WR {wrk})")
        return "\n".join(out)

    avg_ev = sum(float(t.get("ev", 0) or 0) for t in trades) / total

    return (
        f"📓 <b>ALPHA JOURNAL (7D) v16</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"Total <code>{total}</code> · WR(TP1) <b>{wr:.0f}%</b> · EV̄ <code>{avg_ev:+.2f}R</code>\n"
        f"✅TP1 <code>{tp1_n}</code> · 🚀TP2 <code>{tp2_n}</code> · 🏆TP3 <code>{tp3_n}</code>\n"
        f"❌SL <code>{sl_n}</code> · 🔒BE <code>{be_n}</code> · ⏳Open <code>{open_n}</code>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"<b>⏱ Engine:</b>\n<code>{safe_html(breakdown('timeframe'), 300)}</code>\n\n"
        f"<b>🕐 Session:</b>\n<code>{safe_html(breakdown('session'), 300)}</code>\n\n"
        f"<b>🐳 Whale:</b>\n<code>{safe_html(breakdown('whale'), 300)}</code>"
    )
...
class RenderHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"ALPHA v16.0 ULTIMATE ACTIVE")
    def do_HEAD(self):
        self.send_response(200)
...
if __name__ == "__main__":
    ...
    sess_n, _, sess_e = get_trading_session()
    alert_admin(
        f"🏴‍☠️ ALPHA v16.0 ULTIMATE DEPLOYED\n"
