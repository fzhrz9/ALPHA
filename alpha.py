...
def analyze_smc_pa(sym, verbose=True):
    ...
    # 5. FIBONACCI LEVELS
    fib_500 = swing_high - (rng * 0.500)   # 50.0% retracement
    fib_618 = swing_high - (rng * 0.618)   # 61.8% retracement — golden ratio OB zone
    fib_786 = swing_high - (rng * 0.786)   # 78.6% retracement — deepest discount
    ...
    # 6. ANTI-FOMO: WAJIB di Discount Zone (50%–78.6%)
    in_discount = fib_786 <= price <= fib_500
    if not in_discount:
        log("❌ REJECT: " + ("PREMIUM ZONE" if price > fib_500 else "EXTREME (falling knife)"))
        return None
    log(f"✅ FIBO PASS: ${fmt(price)} dalam DISCOUNT ZONE ({fib_zone})")
    ...
    return {
        ...
        "fib_zone": fib_zone,
        "fib_500": fib_500,   # [FIX-7] Exposed untuk pullback watchlist
...
def send_signal(sym, smc_data, vol_24h, btc_chg=0.0):
    ...
    # [FIX-8] MINIMUM RR FILTER: Signal tidak layak jika RR1 < 1.5
    rr1 = (tp1 - entry) / risk if risk > 0 else 0
    if rr1 < 1.5:
        print(f"[SKIP] {sym}: RR1 terlalu rendah ({rr1:.2f} < 1.5 minimum)")
        return False
    ...
    for sym, data in items:
        try:
            ...
            if bos_type == "HL" and current_price < level:
                # Bearish BOS (HL broken) — short bias
                print(f"[{sym}] 💥 BOS BREAK! HL ${fmt(level)} ditembusi @ ${fmt(current_price)} ({elapsed_hours:.1f}h)")
                smc_bos = {
                    "setup": "💥 BOS BREAK (HL Tembus - Bearish)", "entry": current_price,
                    "sl": level * 1.02, "tp1": current_price * 0.95,
                    "tp2": current_price * 0.90, "tp3": current_price * 0.85,
                    "rr1": 2.5, "rr2": 5.0, "score": 4, "fib_zone": "N/A",
