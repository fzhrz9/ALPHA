...
class SignalFormatter:
    ...
    @staticmethod
    def format_premium_signal(symbol, chain, price, vwap, sl, tp1, tp2, tp3, pool_address, token_address):
...
async def run_bot():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        pending_on_start = db.pending_count()
        if pending_on_start > 0:
            logging.info(f"💾 DB selamat — {pending_on_start} token masih dalam pending queue dari sesi lepas")
        else:
            logging.info(f"💾 DB kosong — sesi baru atau DB telah dipadam semasa offline")
        ...
        while True:
            try:
                ...
                # ==========================================
                # 1b. PROMOTER (setiap 30s — berasingan dari scanner)
                # ==========================================
                if now - last_promote >= 30.0:
                    last_promote = now
                    promoted = db.promote_ready_pending()
                    if promoted:
                        for sym, chn in promoted:
                            logging.info(f"   🚀 PROMOTED: {sym} ({chn.upper()}) — umur >= 1h, masuk watchlist")
                        logging.info(f"   📋 Watchlist kini: {len(db.get())} token | Pending: {db.pending_count()}")
                ...
                if now - last_gecko >= GECKO_LIMIT:
                    ...
                    for pool, chain, sym, token_address in wl:
                        ...
                        if a == 0:
                            logging.warning(f"   ⚠️ ATR=0 for {sym} — insufficient candle history (need ≥15), skipping...")
                        ...
                        if spring and price <= v * 1.02:
                            ...
                            if safe:
                                ...
                                msg, keyboard = SignalFormatter.format_premium_signal(
                                    sym, chain, price, v, sl, tp1, tp2, tp3, pool, token_address
                                )
                ...
                if now - last_status >= 60.0:
                    ...
                    if stale:
                        logging.info(f"🧹 Cleaned {stale} stale tokens from watchlist (>6h no signal)")
                    logging.info(f"📊 STATUS: Watchlist={len(db.get())} | Pending={db.pending_count()} | Uptime={int(time.time()-start_time)}s")
