"""
ALPHA — Gate.io Dual Engine Sniper (FINAL INSTITUTIONAL GRADE)
Semua 7 Patch Integrated: Fractal, True EMA, Flexible Structure, Soft H4, VPA 0.7, 
Engine 2 Institutional, HTF Bias, ATR Threshold, Pullback Watchlist, Thread Locks.
"""
...
# =================================================================
# 5. ENGINE 1: PULLBACK SMC (H1 + H4) — INSTITUTIONAL GRADE
# =================================================================
def analyze_smc_pa(sym, verbose=True):
    ...
    # Patch 2: Soften H4 Filter (Guna Structure)
    ...
    if len(candles_h4) >= 20:
        ...
        elif h4_structure == 'unknown': log("⚠️ H4 structure unknown, skip H4 filter")

    # Patch 1: Fractal Swings + Fallback
    swings = find_fractal_swings(candles, lookback=2)
    if len(swings) < 4:
        log("⚠️ Fractal swings tidak cukup, guna EMA trend detection fallback")
    ...
    if structure == 'downtrend': log(f"❌ REJECT: Market structure downtrend (LH/LL)"); return None
    ...
    fib_500 = swing_high - (rng * 0.500); fib_786 = swing_high - (rng * 0.786); fib_zone = f"{fmt(fib_500)} - {fmt(fib_786)}"
    ...
    # Patch 3: True EMA & VPA Impulse vs Pullback (0.7)
    closes = [c['c'] for c in candles[-200:]]
    ema20 = calculate_ema(closes, 20); ema50 = calculate_ema(closes, 50)
    is_uptrend = ema20 > ema50
    if not is_uptrend: log("❌ REJECT: EMA20 < EMA50 (bukan uptrend)"); return None
    
    atr = calculate_atr(candles, 14); distance_from_ema = abs(price - ema20); threshold = atr * 0.5 if atr > 0 else ema20 * 0.015
    if price < ema20 * 0.90: return None
    ...
    if curr['l'] < swing_low and curr['c'] > swing_low and wick_ratio >= 2.0 and touches >= 2 and curr['v'] > avg_impulse_vol:
        setup_name = "💧 LIQUIDITY SWEEP"; score += 3; log("✅ SETUP 7: Sweep")
    ...
    is_engulfing = curr['c'] > curr['o'] and prev['c'] < prev['o'] and curr['c'] > prev['o'] and curr['o'] <= prev['c'] and curr_body > prev_body and curr['v'] > prev['v']
    ...
    if vpa_dry: score += 1; log("✅ VPA PASS")
    else: log("⚠️ VPA WEAK (Optional)")
    ...
    if len(shs) >= 2:
        last_lh = shs[-1]['price']; distance = abs(price - last_lh) / last_lh * 100
        if distance <= 2.0 and price < last_lh:
            with bos_lock:
                if sym not in BOS_WATCHLIST: BOS_WATCHLIST[sym] = {"level": last_lh, "type": "LH", "added": time.time()}

    return {"setup": setup_name, "entry": price, "sl": sl, "tp1": tp1, "tp2": tp2, "tp3": tp3, "rr1": (tp1 - price) / risk, "rr2": (tp2 - price) / risk, "score": score, "fib_zone": fib_zone, "timeframe": "H1", "structure": structure}

# =================================================================
# 6. ENGINE 2: EARLY MOMENTUM (M15) — INSTITUTIONAL GRADE
# =================================================================
def analyze_early_momentum(sym, verbose=True):
    ...
    # Patch 4: Unclosed Candle & H1 Trend Alignment
    curr = candles[-1]
    ...
    vol_spike = curr_vol / avg_vol; log(f"✅ VOLUME ANOMALY: {vol_spike:.1f}x avg")
    ...
    # Patch 4: Close Location & Micro BOS
