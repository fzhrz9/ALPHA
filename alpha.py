"""
ALPHA — Gate.io Triple Engine Sniper v16.5 UI-MINIMALIS

v16.5 UI MINIMALIS MONOKROM (logik dikekalkan 100%):
  Format mesej signal direka semula — gaya bersih profesional:
  header ringkas, gred+metrik 1 baris, ENTRY/SL/TP menegak (label
  huruf besar), analisis teknikal 3-4 baris bullet (•), konteks bawah
  dipisah garis halus. Emoji minimum (⚠️ untuk risiko sahaja).
  Align kiri, monokrom, tiada border. SEMUA engine/math/journal kekal.
...
def send_signal(sym, smc_data, vol_24h, btc_chg=0.0):
    ...
    # ════ [v16.5] MINIMALIS MONOKROM ════
    sy_pass = smc_data.get("syarat_pass", 3)
    grade, _ = compute_grade(sy_pass, ev, whale_signal, choch,
                             session == "DEAD_ZONE")

    # Baris 1 analisis: label syarat pendek dipisah ' · '
    sy_map = smc_data.get("syarat", {})
    sy_tech = " · ".join(SYARAT_SHORT.get(k, k[:5]) for k, v in sy_map.items() if v) or "—"

    # Baris 2 analisis: amaran konteks (huruf kecil, tanpa hingar)
    ctx = []
    if session == "DEAD_ZONE":            ctx.append("⚠️ sesi mati")
    elif session in ("LONDON_OPEN", "NY_OVERLAP"): ctx.append("sesi aktif")
    if whale_signal == "DISTRIBUTING":    ctx.append("⚠️ whale agih")
    elif whale_signal == "LEAN_BEARISH":  ctx.append("whale lemah")
    elif whale_signal in ("ACCUMULATING", "LEAN_BULLISH"): ctx.append("whale beli")
    if choch == "CHOCH_BEAR":             ctx.append("⚠️ struktur retak")
    if smc_data.get("is_counter_trend"):  ctx.append("⚠️ counter-trend")
    ctx_line = " · ".join(ctx) if ctx else "struktur stabil"

    # Baris 3: nama setup (huruf kecil, ringkas)
    setup_clean = safe_html(smc_data.get("setup", ""), 40)

    # Baris konteks bawah: sesi · risk · rrnet
    se_txt = {"LONDON_OPEN": "LONDON", "NY_OVERLAP": "NY-OVERLAP", "NY_SESSION": "NY",
              "ASIA_SESSION": "ASIA", "DEAD_ZONE": "DEAD-ZONE"}.get(session, session)
    zone_txt = smc_data.get("setup_mode", "")
    zone_part = f"{zone_txt} · " if zone_txt and zone_txt not in ("BREAKOUT",) else ""
    ...
    msg = (
        f"ALPHA {engine_label} – {sym}/USDT · {timeframe}\n"
        f"\n"
        f"GRED {grade} · {sy_pass}/5 · {p_win*100:.0f}% · EV {ev:+.2f}R\n"
        f"\n"
        f"{btc_warn}"
        f"ENTRY     <code>${fmt(entry)}</code>\n"
        f"SL        <code>${fmt(sl)}</code>  −{sl_pct:.1f}%\n"
        f"TP1       <code>${fmt(tp1)}</code>  +{tp1_pct:.1f}%  RR {rr1:.1f}\n"
        f"TP2       <code>${fmt(tp2)}</code>  +{tp2_pct:.1f}%\n"
        f"TP3       <code>${fmt(tp3)}</code>  +{tp3_pct:.1f}%\n"
        f"\n"
        f"• {sy_tech}\n"
        f"• {ctx_line}\n"
        f"• {setup_clean}\n"
        f"• {zone_part}{se_txt} · Risk ${risk_usd:.2f} ({user_risk}%) · RRnet {rr1_net:.2f}"
    )
    ...
    try:
        ...
        record = {
            ...
            "confluence": smc_data.get("confluence_labels", "")[:100],
...
class RenderHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"ALPHA v16.5 ACTIVE")
    def do_HEAD(self):
        self.send_response(200)
