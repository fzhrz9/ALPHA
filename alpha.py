...
class DataFetcher:
    ...
    async def ohlcv_h1(self, net, pool):
        """H1 candles — dibina dari minute aggregate=60 (lebih stabil dari /ohlcv/hour)"""
        return await self._ohlcv(net, pool, 'minute', 60, 48)
                ...
                if now - last_gecko >= GECKO_LIMIT:
                    ...
                    for pool, chain, sym, token_address in (wl if all_wl else []):
                        ...
                        if len(h1) < 6 or len(m15) < 20:
                            logging.warning(
                                f"   ⚠️ {sym}: Data kurang (H1:{len(h1)}, M15:{len(m15)}) "
                                f"— token mungkin terlalu baru atau GeckoTerminal tiada data. "
                                f"Buang dari watchlist."
                            )
                            db.remove(pool)   # buang token yang tiada data supaya tidak block giliran lain
                            continue
