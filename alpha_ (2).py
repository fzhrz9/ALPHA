...
def find_fractal_swings(candles, lookback=2):
    ...
    for i in range(lookback, n - lookback):
        ...
        for j in range(1, lookback + 1):
            if candles[i]['h'] < candles[i - j]['h'] or candles[i]['h'] < candles[i + j]['h']: is_sh = False
            if candles[i]['l'] > candles[i - j]['l'] or candles[i]['l'] > candles[i + j]['l']: is_sl = False
...
def analyze_smc_pa(sym, verbose=True):
    ...
    if risk <= 0:
        sl = price * 0.98
        risk = price - sl
        log(f"⚠️ Risk calculation failed. Forced SL at -2%: ${fmt(sl)}")
...
def analyze_early_momentum(sym, verbose=True):
    ...
    try:
        ...
        if len(candles_h4) >= 20:
            ...
            for close in h4_closes[20:]: h4_ema = (close - h4_ema) * multiplier + h4_ema
            h4_current = candles_h4[-1]['c']
            if h4_current > h4_ema: h4_bias = "uptrend"
    ...
    for t in candidates:
        ...
        if SCAN_MODE in ["momentum", "both"]:
            if t in momentum_candidates:
                ...
                if (current_time - curr['t']) < (15 * 60 * 0.90): continue

                avg_vol = sum(c['v'] for c in candles_m15[-20:-1]) / 19
                curr_vol = curr['v']
                
                if curr_vol >= (avg_vol * 2.5): # RELAXED: 2.5x
                    ...
                    if curr['c'] < (range_low + (total_range * 0.60)): # RELAXED: 0.60
                    ...
                    if curr['c'] <= local_lh and curr_vol/avg_vol < 4.0: # RELAXED: 4.0x exception
                        if sym in WATCHLIST: del WATCHLIST[sym]
                        continue
                    ...
                    if range_pct <= 8 and (is_pinbar or is_engulfing): # RELAXED: 8%
                        smc_m15 = {
...
def monitor_bos_breaks():
    if not IS_SCANNING or not BOS_WATCHLIST: return
    symbols_to_remove = []
...
def cmd_start(msg):
    ...
    preset_lbl = PRESETS.get(cfg.get("active_preset", "standard"), {}).get("label", "Custom")
    text = (
        f"🏴☠️ <b>ALPHA — Dual Engine Sniper (CLEAN)</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
...
def cmd_tune(msg):
    ...
    if len(parts) < 2:
        ...
        text = (
            ...
            f"🟢 <code>/tune soft</code> — Vol $300K, Pass 2\n"
...
class RenderHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"ALPHA DUAL ENGINE v3 ACTIVE")
    def do_HEAD(self):
        self.send_response(200)
...
if __name__ == "__main__":
    ...
    time.sleep(5)
    alert_admin(
        f"🏴‍☠️ ALPHA Dual Engine v3 DEPLOYED (CLEAN & FIXED)\n"
