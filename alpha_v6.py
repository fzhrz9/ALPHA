...
class DataFetcher:
    ...
    async def ohlcv_h1(self, net, pool):
        ...
        Jika kurang dari 6 candle, bina dari M15 (resample 4×M15=1×H1).
        """
        h1 = await self._ohlcv(net, pool, 'hour', 1, 48)
        if len(h1) >= 6:
            return h1
        # Fallback: resample dari M15 (96 candle M15 = 24 candle H1)
        m15 = await self._ohlcv(net, pool, 'minute', 15, 96)
        await asyncio.sleep(2.0)
        if len(m15) >= 4:
            h1_resampled = DataFetcher.resample_m15_to_h1(m15)
            if len(h1_resampled) >= 6:
                logging.info(f"      ♻️ H1 dibina dari M15 resample ({len(m15)} M15 → {len(h1_resampled)} H1)")
                return h1_resampled
        return []
