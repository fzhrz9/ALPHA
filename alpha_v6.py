...
# ==========================================
# FILTER CONFIG — boleh tukar via /start
# ==========================================
FILTER_PRESETS = {
    'ketat': {
        'liq_min':  50000,   # $50K
        ...
        'age_max':  120,     # 5 hari
        'rr_min':   2.0,     # RR >= 2x
        'bo_vol_x': 2.0,     # Volume breakout 2x
    },
    'sederhana': {
        'liq_min':  30000,
        'vol_min':  50000,
...
# Default aktif
active_filter = FILTER_PRESETS['sederhana'].copy()
active_preset_name = 'sederhana'
...
# ==========================================
# 4. COMMAND HANDLER (/start, /filter, /status)
# ==========================================
class CommandHandler:
    ...
    async def poll_updates(self):
        url = self.base_url + "/getUpdates?timeout=2&offset=" + str(self.offset)
        try:
    ...
    async def _handle(self, update):
        global active_filter, active_preset_name
        ...
        if text in ("/start", "/menu"):
            await self._menu(cid)
        elif text in ("/ketat", "/sederhana", "/longgar"):
            preset = text[1:]
            active_filter = FILTER_PRESETS[preset].copy()
            active_preset_name = preset
            icon = {"ketat": "🔴", "sederhana": "🟡", "longgar": "🟢"}[preset]
            f = active_filter
            lines = [
                icon + " <b>Filter: " + preset.upper() + "</b>",
                "Liq  : <code>$" + f"{f['liq_min']:,}" + "</code>",
                "Vol  : <code>$" + f"{f['vol_min']:,}" + "</code>",
                "MC   : <code>$" + f"{f['mc_min']:,}" + " - $" + f"{f['mc_max']:,}" + "</code>",
                "Umur : <code>" + str(f['age_min']) + "h - " + str(f['age_max']) + "h</code>",
                "RR   : <code>" + str(f['rr_min']) + "x</code>",
                "BO   : <code>" + str(f['bo_vol_x']) + "x vol</code>",
                "",
                "Aktif pada scan seterusnya.",
            ]
            await self._reply(cid, "\n".join(lines))
        elif text == "/status":
            await self._status(cid)
        elif text == "/clear":
            wl = self.db.get()
            for row in wl:
                self.db.remove(row[0])
            await self._reply(cid, "<b>" + str(len(wl)) + " token dibuang.</b>")
        elif text == "/help":
            await self._help(cid)

    async def _menu(self, cid):
        f = active_filter
        icon = {"ketat": "🔴", "sederhana": "🟡", "longgar": "🟢"}.get(active_preset_name, "🟡")
        wl_count = len(self.db.get())
        up = int(time.time() - start_time)
        h, m2 = divmod(up, 3600)
        m2, s = divmod(m2, 60)
        lines = [
            "🤖 <b>ALPHA SIGNAL BOT v3.0</b>",
            "━━━━━━━━━━━━━━━━━━━━━",
            "Uptime    : <code>" + str(h) + "h " + str(m2) + "m " + str(s) + "s</code>",
            "Watchlist : <code>" + str(wl_count) + " token</code>",
            icon + " Filter   : <b>" + active_preset_name.upper() + "</b>",
            "Liq $" + f"{f['liq_min']:,}" + "+ | Vol $" + f"{f['vol_min']:,}" + "+",
            "MC $" + f"{f['mc_min']:,}" + " - $" + f"{f['mc_max']:,}",
            "Umur " + str(f['age_min']) + " - " + str(f['age_max']) + "h | RR " + str(f['rr_min']) + "x",
            "",
            "<b>Preset:</b>",
            "🟢 /longgar   - Banyak signal",
            "🟡 /sederhana - Seimbang (default)",
            "🔴 /ketat     - Berkualiti tinggi",
            "",
            "/status | /clear | /help",
        ]
        keyboard = [
            [
                {"text": "🟢 Longgar",   "callback_data": "/longgar"},
                {"text": "🟡 Sederhana", "callback_data": "/sederhana"},
                {"text": "🔴 Ketat",     "callback_data": "/ketat"},
            ],
            [
                {"text": "📊 Status",   "callback_data": "/status"},
                {"text": "🧹 Clear WL", "callback_data": "/clear"},
            ]
        ]
        try:
            async with self.session.post(
                self.base_url + "/sendMessage",
                json={"chat_id": cid, "text": "".join(lines), "parse_mode": "HTML",
                      "reply_markup": {"inline_keyboard": keyboard}}
            ) as r:
                pass
        except Exception as e:
            logging.error("Menu error: " + str(e))
    ...
    async def _status(self, cid):
        ...
        lines = [
            "📊 <b>STATUS</b>",
            "Uptime   : <code>" + str(h) + "h " + str(m2) + "m " + str(s) + "s</code>",
            "Watchlist: <code>" + str(len(wl)) + " token</code>",
            icon + " Filter: <b>" + active_preset_name.upper() + "</b>",
            "",
            "<b>Token aktif:</b>",
        ]
        for row in wl[:10]:
            lines.append("  - " + row[2] + " (" + row[1].upper() + ")")
        if len(wl) > 10:
            lines.append("  ... dan " + str(len(wl) - 10) + " lagi")
        if not wl:
            lines.append("  (kosong)")
        await self._reply(cid, "".join(lines))

    async def _help(self, cid):
        lines = [
            "📖 <b>BANTUAN</b>",
            "",
            "🟢 <b>LONGGAR</b>: Liq $15K, Vol $20K, MC $50K-$20M, Umur 2-336h, RR 1.2x",
            "🟡 <b>SEDERHANA</b>: Liq $30K, Vol $50K, MC $100K-$10M, Umur 4-168h, RR 1.5x",
            "🔴 <b>KETAT</b>: Liq $50K, Vol $100K, MC $200K-$5M, Umur 8-120h, RR 2.0x",
            "",
            "<b>Engine:</b>",
            "🚀 BREAKOUT: Tembus rintangan H1 + volume tinggi + CHoCH",
            "🎯 DIP: Pullback ke Fib 0.618-0.786 atau FVG dalam uptrend H1",
            "",
            "<b>Security:</b> GoPlus - honeypot, tax, mintable",
            "<b>Data:</b> GeckoTerminal trending pools H1+M15 realtime",
        ]
        await self._reply(cid, "".join(lines))

    async def _handle_callback(self, update):
        cb    = update.get("callback_query", {})
        data  = cb.get("data", "")
        cid   = str(cb.get("from", {}).get("id", ""))
        cb_id = cb.get("id", "")
        try:
        ...
        if cid != str(ADMIN_CHAT_ID):
            return
        fake = {"message": {"text": data, "chat": {"id": int(cid)}}}
        await self._handle(fake)
    ...
    async def _reply(self, cid, text):
        try:
            ...
            logging.error("Reply error: " + str(e))
...
class QuantMath:
    ...
    def breakout_engine(candles_h1, candles_m15):
        ...
        bo_vol  = bo_candle['v']
        bo_vol_x = active_filter.get('bo_vol_x', 1.5)
        if avg_vol > 0 and bo_vol < bo_vol_x * avg_vol:
        ...
        rr_min = active_filter.get('rr_min', 1.5)
        if rr < rr_min:
            return False, {}, f"Risk-Reward terlalu rendah ({rr:.2f}x < {rr_min}x)"
...
class DataFetcher:
    ...
    @staticmethod
    def resample_m15_to_h1(candles_m15):
        """
        Bina H1 dari M15 — setiap 4 candle M15 = 1 candle H1.
        Formula: open=first, high=max, low=min, close=last, vol=sum
        Ini kaedah yang sama TradingView guna untuk resample.
        """
        if len(candles_m15) < 4:
            return []
        h1 = []
        for i in range(0, len(candles_m15) - 3, 4):
            group = candles_m15[i:i+4]
            if len(group) < 4:
                break
            h1.append({
                't': group[0]['t'],
    ...
    async def ohlcv_h1(self, net, pool, existing_m15=None):
        """
        H1 candles — cuba /ohlcv/hour dahulu.
        Jika kurang dari 6 candle, resample dari M15 yang SUDAH ADA
        (bukan fetch semula — elak rate limit call ke-3).
        """
        h1 = await self._ohlcv(net, pool, 'hour', 1, 48)
        if len(h1) >= 6:
            logging.info(f"      📊 H1 dari endpoint hour ({len(h1)} candle)")
            return h1

        # Guna M15 yang dah difetch — JANGAN fetch semula
        m15 = existing_m15 if existing_m15 and len(existing_m15) >= 4 else []
        if not m15:
            logging.warning(f"      ⚠️ H1/hour kosong dan tiada M15 untuk resample")
            return []

        h1_resampled = DataFetcher.resample_m15_to_h1(m15)
        if len(h1_resampled) >= 6:
            logging.info(f"      ♻️ H1 dari resample: {len(m15)} M15 → {len(h1_resampled)} H1")
            return h1_resampled

        logging.warning(f"      ⚠️ Resample gagal: M15:{len(m15)} → H1:{len(h1_resampled)} (kurang 6)")
        return []
...
async def run_bot():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        db       = Database()
        cmd      = CommandHandler(session, db)

        await tg.send_admin_alert(
            f"✅ Bot v3.0 Online | Breakout + Dip Engine\n"
            f"Filter: {active_preset_name.upper()} | /start untuk menu", "INFO"
        )
        wl_count = len(db.get())
        ...
        last_gecko  = 0
        last_status = 0
        analyze_idx = 0   # pointer giliran — rotate semua token
            ...
            try:
                ...
                if now - last_scan >= 300.0:
                    ...
                    for p in pools:
                        ...
                        if liq  < f['liq_min']:                        continue
                        if vol  < f['vol_min']:                        continue
                        if mc   < f['mc_min'] or mc > f['mc_max']:    continue
                        if age  < f['age_min'] or age > f['age_max']: continue
                ...
                if now - last_gecko >= GECKO_LIMIT:
                    ...
                    all_wl = db.get()
                    if not all_wl:
                        logging.info(f"📊 Watchlist kosong | Menunggu scanner...")
                    else:
                        # Rotate giliran — analisis 1 token per kitaran secara bergilir
                        analyze_idx = analyze_idx % len(all_wl)
                        wl = [all_wl[analyze_idx]]
                        analyze_idx += 1
                        logging.info(f"📊 Giliran {analyze_idx}/{len(all_wl)} | {wl[0][2]} ({wl[0][1].upper()}) | Watchlist: {len(all_wl)}")

                    for pool, chain, sym, token_address in (wl if all_wl else []):
                        logging.info(f"   📈 {sym} ({chain.upper()}) — Ambil M15...")
                        m15 = await fetcher.ohlcv_m15(chain, pool)
                        await asyncio.sleep(2.0)

                        logging.info(f"   📈 {sym} ({chain.upper()}) — Ambil H1... (M15:{len(m15)})")
                        h1  = await fetcher.ohlcv_h1(chain, pool, existing_m15=m15)
                        await asyncio.sleep(2.0)

                        if len(h1) < 6 or len(m15) < 20:
                            logging.warning(
                                f"   ⚠️ {sym}: Data kurang (H1:{len(h1)}, M15:{len(m15)}) — skip, cuba semula kitaran akan datang"
                            )
                            continue  # skip sahaja, jangan buang — data mungkin ada nanti
                ...
                # Poll Telegram commands setiap 2s
                await cmd.poll_updates()
                await asyncio.sleep(0.5)
